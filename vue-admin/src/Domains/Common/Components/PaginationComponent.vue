...
export default {
    ...
    computed: {
        ...
        full_url() {
            ...
            return this.computed_url.replace(/\?$/) + '?' + qs.stringify(params);
        },
    ...
    methods: {
        changePerPage() {
            ...
            let url = this.computed_url.replace(/\?$/) + '?' + qs.stringify(params);
            this.$router.push(`${url}`)
        },
    },
    mounted() {
        if ( !StorageHandler.getItem(`${this.computed_url}__per_page`) ) {
            StorageHandler.setItem(`${this.computed_url}__per_page`, this.per_page_options[0]);
        }

        this.per_page = StorageHandler.getItem(`${this.computed_url}__per_page`);
    },
}
</script>
...
<div class="pagination">
    <div class="flex items-center" v-if="show_entries_count">
        <div>Showing {{ per_page * (page - 1) + 1 }} - {{ visible_entries }} of {{ total }} entries</div>
    ...
    <div class="flex items-center" v-if="pages > 1">
        ...
        <div class="pages">
            <template v-for="i in visible_pages">
                <div class="page page--current" :key="i" v-if="i == page">{{ i }}</div>
